...
export class UserController {
  ...
  static async updateProfile(req: AuthRequest, res: Response) {
    try {
      const userId = req.user!.id;
      const { firstName, lastName, phoneNumber, avatarUrl, selectedSubjects, aspiringCourse, goalScore, learningStyle, diagnosticResults, preferredStudyTime, studyReminders, examYear, dateOfBirth, state, school } = req.body;
      ...
      // Validate diagnosticResults if provided
  ...
  static async getGoalProgress(req: AuthRequest, res: Response) {
    try {
      ...
      const goalProgress = AnalyticsService.calculateGoalProgress(user.goalScore, quizAttempts);
